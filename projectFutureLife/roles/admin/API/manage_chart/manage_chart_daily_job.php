...
// SQL
$sql = "SELECT MONTH(time_start) as month, COUNT(*) as total_job FROM daily_job WHERE status = " . FLG_ON . " AND YEAR(time_start) = ? GROUP BY MONTH(time_start)";
...
$data = [
    ...
    "datasets" => [
        [
            "label" => "Số lượng công việc được thực hiện trong tháng",
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $data["datasets"][0]["data"][$month] = $row["total_job"];
    }
}
