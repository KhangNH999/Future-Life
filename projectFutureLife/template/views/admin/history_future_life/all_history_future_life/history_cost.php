...
$show_history_cost = $history_future_life->show_history_cost();
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $action = $_GET['action'];
    $delete_history_cost = $history_future_life->delete_history($id, $action);
}
?>
...
<div class="title-log"><i class="fa fa-folder-open"></i> Lịch sử chi tiêu</div>
<?php
if ($show_history_cost) {
    while($result = $show_history_cost->fetch_assoc()){
?>
<div class="log_file">
    <div class="sub_log">
        <div class="log_info">
        ...
        <div class="delete_log"><a href="admin_cp.php?action=history_cost&query=show&id=<?php echo $result['id'] ?>" onclick="return confirm_delete(event);"><i class="fa fa-remove"></i></a></div>
    </div>
</div>
<?php
    }
