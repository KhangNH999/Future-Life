...
$show_history_future_plan = $history_future_life->show_history_future_plan();
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $action = $_GET['action'];
    $delete_history_future_plan = $history_future_life->delete_history($id, $action);
}
?>
...
<div class="title-log"><i class="fa fa-folder-open"></i> Lịch sử dự định tương lai</div>
<?php
if ($show_history_future_plan) {
    while($result = $show_history_future_plan->fetch_assoc()){
?>
<div class="log_file">
    <div class="sub_log">
        <div class="log_info">
        ...
        <div class="delete_log"><a href="admin_cp.php?action=history_future_plan&query=show&id=<?php echo $result['id'] ?>" onclick="return confirm_delete(event);"><i class="fa fa-remove"></i></a></div>
    </div>
</div>
<?php
    }
