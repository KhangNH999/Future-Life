...
<?php
  ...
  $row_count = $future_plan->get_count_future_plan($id, $future_plan_name, $time_start)->num_rows;
  ...
  // Update status future plan
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn-update-status-future-plan'])) {
    $future_plan = new future_plan();
    if (!empty($_POST['future_plan_ids'])) {
      $selected_ids = $_POST['future_plan_ids'];
      foreach ($selected_ids as $id_future_plan) {
        $future_plan->update_status($id_future_plan);
      }
    }
  }
?>
<form action="admin_cp.php?action=manage_future_plan&query=show" method="post">
...
<form action="" method="post">
  ...
  <button class="button-export-excel" type="submit" name="button-export-excel-future-plan">Xuất Excel</button>
</form>

<form action="" method="post">
  <button class="button-update-status" type="submit" name="btn-update-status-future-plan">Cập nhật tình trạng</button>
  <br>
  <br>
  <div class="manage_table">
    ...
    ?>
      <tr>
        <td style="text-align:center;"><input type="checkbox" name="future_plan_ids[]" value="<?php echo $result['id'] ?>"></td>
